...
- Nothing content-bearing (prompts, diffs, bodies, messages) may cross the
  attestation wire. Only metadata may cross it. See D-0001.
...
- Never create the Windows control pipe without its explicit security descriptor
  (user-only DACL + medium integrity label). Set it in `bind` **and** in the
  accept loop, or connection #2 silently loses it. Never report
  `ERROR_ACCESS_DENIED` as "daemon not running". See D-0016.
