...
`first_commit` — never passed in, never taken from the ambient session, and set
to NULL when that commit was never captured.
...
itself — and that row ships to the cloud, where nothing re-derives it. Locally
...
a recorded fact keyed by the commit itself — the same join the local read path
already trusts.
...
git resolves to the second — the walk wins, because it enumerates history in
order and the timestamp has lost that.
