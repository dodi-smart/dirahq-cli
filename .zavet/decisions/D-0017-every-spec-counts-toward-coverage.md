...
`coverage_globs` is the coverage surface: active decisions' guard globs ∪
...
specs sit at `verified: false` — including one at `confidence: high` — so the
old gate meant specs contributed **exactly zero** to a number labelled
coverage. It read as "guards only" and silently understated the work.
...
statement about the code's current surface (a superseded guard enforces
...
- Count only specs whose paths still exist — conflates coverage with staleness,
  which specs already report separately (`stale_commits`).

## Agent directives

- Do NOT re-add a `verified` filter to the coverage surface. It looks like a
  missing honesty check; it is not. Unverified knowledge is still knowledge,
  and `verified` is already surfaced per record for anyone who wants to weigh
  it.
- A new record type joining the coverage surface goes through `coverage_globs`
