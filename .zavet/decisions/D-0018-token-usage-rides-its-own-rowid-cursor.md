...
watermark advanced on the final chunk's 2xx. That is the same discipline
...
- `capture_tokens` runs on `Stop`/`SessionEnd` and stamps each turn with the
  **transcript's** timestamp, so a turn is always dated *before* the event that
  discovered it. That falls below the window's own exclusive lower bound.
...
Measured on the reporting machine, 3,471 of 3,656 rows were unshippable:
$3,063 of $3,148, or **97.3%**. The dashboard read `$2` for a week that cost
~$978.
...
is the one direction that id cannot protect against. That is why the fix is a
cursor, not a wider window.

## Rejected

- **Widen the `at` window** (inclusive bound, previous window's upper bound as
  the lower one). This leaves the no-cursor and no-catch-up failure modes
  intact, and keeps both the TEXT-comparison and empty-`at` hazards.
- **Initialise the cursor to the current max rowid on upgrade**. This looks
  correct but is wrong: it strands exactly the backlog the change exists to
  recover. An absent key reads as "from the beginning", which is the
  behaviour we want.
- **A separate token flush loop**: a second scheduler, a second backoff and a
  second failure mode, to solve a problem the existing chunking already
  handles.
