...
across the write **and** across every `spawn` in the file — via
...
opens `A/bin/dira` to stage it; thread B forks and its child inherits A's write
fd — `O_CLOEXEC` only applies on a *successful* exec — and pins the inode until
it exits; A then execs and gets `ETXTBSY`. Per-test tempdirs do not help, and
neither does write-then-rename: renaming gives the inode another name while the
inherited fd still refers to it. The window has to close on the fork side.
...
`42.0.0 is available` — a release that does not exist. The suite was green
throughout; the damage only surfaced on an unrelated invocation.
...
moment `8ae49e1` deleted it — which is the failure mode that actually occurred.
